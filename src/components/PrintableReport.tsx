...
export const PrintableReport: React.FC<PrintableReportProps> = ({
  ...
  // Group QA Review Tasks by Developer + Date + Project Code
  const groupedQAReviewTasks = qaReviewTasks.reduce<{ developer: string; date: string; projectCode: string; items: typeof qaReviewTasks }[]>((groups, item) => {
    const existingGroup = groups.find(g => g.developer === item.developer && g.date === item.date && g.projectCode === item.projectCode);
    if (existingGroup) {
      existingGroup.items.push(item);
    } else {
      groups.push({ developer: item.developer, date: item.date, projectCode: item.projectCode, items: [item] });
    }
    return groups;
  }, []);
  groupedQAReviewTasks.sort((a, b) => (a.date || '').localeCompare(b.date || ''));

  // Group Unfinished Tasks by Developer + Date
  const groupedUnfinishedTasks = unfinishedTasks.reduce<{ developer: string; date: string; items: typeof unfinishedTasks }[]>((groups, item) => {
    const existingGroup = groups.find(g => g.developer === item.developer && g.date === item.date);
    if (existingGroup) {
      existingGroup.items.push(item);
    } else {
      groups.push({ developer: item.developer, date: item.date, items: [item] });
    }
    return groups;
  }, []);
  ...
  return (
    <div className="paper-document-outer-wrapper w-full flex flex-col items-center space-y-8 print:space-y-0 print:m-0 print:p-0">
      ...
      {/* Printable Document Canvas Wrap */}
      <div className="w-full max-w-[794px] flex flex-col items-center print:m-0 print:p-0">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col justify-between min-h-[920px] space-y-6">
            
            <div className="space-y-6">
              
              {/* CORPORATE LETTERHEAD HEADER IMAGE */}
              {templateConfig.showLetterhead && (
                <div className="document-header-letterhead pb-4 border-b border-slate-400 print:break-inside-avoid">
                ...
                {groupedDoneTasks.length === 0 ? (
                  ...
                  <div className="space-y-2.5 text-xs text-slate-900">
                    {groupedDoneTasks.map((group, gIdx) => (
                      <div key={gIdx} className="task-item-group space-y-1">
                          ...
                          {group.items.map((item, idx) => (
                            <li key={idx} className="space-y-0.5">
                              ...
                              {item.task.evidenceUrl && (
                                <div className="pl-5 pt-0.5 text-xs text-slate-800 evidence-block">
              ...
              <section className="space-y-2">
                    ...
                    {groupedQAReviewTasks.map((group, gIdx) => (
                      <div key={gIdx} className="task-item-group space-y-1">
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            <strong className="text-slate-900">
                              {group.developer ? `${group.developer} (${group.projectCode}${group.date ? ` - ${formatDateShort(group.date)}` : ''})` : `${group.projectCode}${group.date ? ` (${formatDateShort(group.date)})` : ''}`}:
                            </strong>
                          </div>
                        </div>
                        <ul className="space-y-1.5 pl-5 text-xs text-slate-900">
                ...
                {/* QA Signature Lines */}
                <div className="signature-block pt-2 space-y-0.5 text-xs font-semibold text-slate-900">
                    ...
                    {groupedUnfinishedTasks.map((group, gIdx) => (
                      <div key={gIdx} className="task-item-group space-y-1">
                        <div className="flex items-start space-x-2">
                          <span className="text-slate-500 font-mono">o</span>
                          <strong className="text-slate-900">
                            {group.developer ? `${group.developer}${group.date ? ` (${formatDateShort(group.date)})` : ''}` : formatDateShort(group.date)}:
                          </strong>
                        </div>
                        <ul className="space-y-1.5 pl-5 text-xs text-slate-900">
                          {group.items.map((item, idx) => (
            ...
            {/* Corporate Footer */}
            <div className="document-footer-letterhead pt-4 border-t border-slate-400 text-[11px] text-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 font-sans print:break-inside-avoid">
